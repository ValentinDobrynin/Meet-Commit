...
async def admin_help_handler(message: Message) -> None:
    ...
    help_text = (
        ...
        "📋 <b>Очистка Review Queue:</b>\n"
        "🧹 <code>/review_clean [режим]</code> - Очистка старых записей и дубликатов\n"
        "   💡 <i>Пример: /review_clean old days=7</i>\n"
        "❓ <code>/review_clean_help</code> - Подробная справка по очистке\n"
        "📊 <code>/review_stats</code> - Расширенная статистика Review Queue\n\n"
        ...
        "• Команды очистки используют dry-run режим по умолчанию\n"
        "• Большие операции могут занять несколько минут\n"
        "• Операции архивирования необратимы (кроме dry-run)\n"
...
async def clients_cleanup_handler(message: Message) -> None:
    ...
    try:
        ...
        await message.answer(cleanup_text)
...
async def people_activity_handler(message: Message) -> None:
    ...
    try:
        ...
        await message.answer(activity_text)
