...
async def performance_report_handler(message: Message) -> None:
    ...
    try:
        ...
        await message.answer(f"❌ <b>Ошибка отчета производительности</b>\\n\\n<code>{str(e)}</code>")
    ...
    try:
        ...
        if bottlenecks["slow_operations"]:
            ...
            for op in bottlenecks["slow_operations"][:5]:
                ...
                lines.append(f"{severity_emoji} <code>{op['operation']}</code>: {op['avg_ms']:.1f}ms")
            lines.append("")
        ...
        if bottlenecks["high_concurrency"]:
            ...
            for op in bottlenecks["high_concurrency"][:3]:
                lines.append(f"🔥 <code>{op['operation']}</code>: пик {op['peak_concurrent']} одновременно")
            lines.append("")
...
async def benchmark_handler(message: Message) -> None:
    ...
    try:
        ...
        status_msg = await message.answer(f"🏃 <b>Запускаю бенчмарк: {benchmark_type}</b>\\n\\n⏳ Это может занять 10-30 секунд...")
