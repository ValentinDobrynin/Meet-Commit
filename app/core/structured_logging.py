...
def get_meeting_logger(
    meeting_id: str | None = None, user_id: int | None = None
) -> StructuredLogger:
...
def log_commit_operation(
    ...
    # Маппинг операций к EventType
    if operation == "created":
        event_type: EventType = "commit_created"
    elif operation == "updated":
        event_type = "commit_updated"
    else:
        event_type = "commit_extracted"  # Fallback

    logger.info(event_type, f"Commit {operation}: {commit.get('title', 'Unknown')}", **extra)
...
def log_meeting_operation(
    ...
    logger.info(event_type, f"Meeting {operation}: {meeting.get('title', 'Unknown')}", **extra)
...
def log_llm_response(
    ...
    extra: dict[str, Any] = {"response_length": response_length}
    if tokens_used:
...
def log_notion_response(
    ...
    extra: dict[str, Any] = {"results_count": results_count}
    if duration_ms:
        extra["duration_ms"] = duration_ms
...
def log_pipeline_stage(
    ...
    **extra: Any,
    ...
    # Используем ближайший подходящий EventType
    logger.info("pipeline_start", f"Pipeline stage: {stage}", **log_extra)
