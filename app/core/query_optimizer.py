...
def _generate_cache_key(
    database_id: str, filter_: dict[str, Any] | None, sorts: list[dict] | None
) -> str:
    ...
    return hashlib.md5(cache_str.encode(), usedforsecurity=False).hexdigest()  # nosec B324
...
def get_cache_stats() -> dict[str, Any]:
    ...
    time.time()
    valid_entries = sum(1 for _, timestamp in _query_cache.values() if _is_cache_valid(timestamp))
...
class NotionQueryOptimizer:
    ...
    async def query_database(
        ...
        try:
            ...
            logger.debug(
                f"Database query completed in {duration_ms:.1f}ms, returned {len(result.get('results', []))} items"
            )
            return result
...
async def paginate_all_results(
    ...
    logger.info(
        f"Pagination completed: {len(all_results)} total results from {page_count + 1} pages"
    )
    return all_results
