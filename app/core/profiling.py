...
class PerformanceProfiler:
    ...
    def __enter__(self) -> PerformanceProfiler:
        ...
        _peak_concurrency[self.operation_name] = max(
            _peak_concurrency[self.operation_name], _concurrent_operations[self.operation_name]
        )
        return self
...
async def async_profiler(operation_name: str):
    ...
    _peak_concurrency[operation_name] = max(
        _peak_concurrency[operation_name], _concurrent_operations[operation_name]
    )
...
def get_performance_summary() -> dict[str, Any]:
    ...
    return {
        ...
        "top_operations_by_time": sorted(
            all_stats.values(), key=lambda x: x["total_ms"], reverse=True
        )[:10],
        "top_operations_by_calls": sorted(
            all_stats.values(), key=lambda x: x["calls"], reverse=True
        )[:10],
    }
...
async def benchmark_async_vs_sync(
    ...
    stats = {
        ...
        "comparison": {
            ...
            "efficiency_gain": ((sync_duration - async_duration) / sync_duration) * 100
            if sync_duration > 0
            else 0,
        },
    }
    ...
    logger.info(
        ...
        f"async {stats['async']['duration_ms']:.1f}ms vs sync {stats['sync']['duration_ms']:.1f}ms "  # type: ignore[index]
...
def detect_bottlenecks(min_calls: int = 10) -> dict[str, Any]:
            ...
            bottlenecks["high_concurrency"].append(
                {
                    ...
                    "peak_concurrent": peak,
                    "total_calls": _call_counts[operation],
                    "avg_concurrent": peak / _call_counts[operation]
                    if _call_counts[operation] > 0
                    else 0,
                }
            )
    ...
    if bottlenecks["slow_operations"]:
        recommendations.append(
            "Оптимизировать медленные операции: добавить кэширование или уменьшить timeout'ы"
        )

    if bottlenecks["high_concurrency"]:
        recommendations.append(
            "Добавить семафоры для контроля конкурентности в высоконагруженных операциях"
        )
...
def generate_performance_report() -> str:
    ...
    if bottlenecks["slow_operations"]:
        ...
        for bottleneck in bottlenecks["slow_operations"][:3]:
            ...
            report.append(
                f"{severity} {bottleneck['operation']}: {bottleneck['avg_ms']:.1f}ms среднее"
            )
        report.append("")
