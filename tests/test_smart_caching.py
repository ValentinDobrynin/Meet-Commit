...
class TestPerformanceImprovements:
    ...
    @patch("app.core.clients.settings")
    @pytest.mark.xfail(
        reason="get_notion_http_client не кэшируется — создаётся новый клиент каждый раз для lifecycle безопасности"
    )
    ...
    @pytest.mark.xfail(
        reason="get_notion_http_client не имеет cache_info — намеренно не кэшируется"
    )
...
class TestRealWorldScenarios:
    ...
    @pytest.mark.xfail(
        reason="get_notion_http_client не кэшируется — производительность теста зависит от системы"
    )
    @patch("app.core.clients.settings")
    ...
    @pytest.mark.xfail(
        reason="get_notion_http_client не кэшируется — каждый вызов создаёт новый клиент"
    )
