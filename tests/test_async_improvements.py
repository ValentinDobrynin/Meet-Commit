...
from app.core.llm_commit_parse_async import parse_commit_text_async
...
class TestAsyncLLMCommitParse:
    ...
    @pytest.mark.asyncio
    @patch("app.core.llm_commit_parse_async.get_async_openai_client")
    async def test_parse_commit_text_async_success(self, mock_get_client):
        """Тест успешного асинхронного парсинга."""
        # Мокаем LLM ответ
        mock_client = AsyncMock()
        mock_response = Mock()
        mock_response.choices = [Mock()]
        mock_response.choices[0].message.content = '{"text": "подготовить отчет", "assignee": "Sasha", "confidence": 0.8}'
        ...
        with patch("app.core.llm_commit_parse_async._load_prompt", return_value="test prompt"):
            with patch("app.core.commit_normalize.build_title", return_value="Test Title"):
                with patch("app.core.commit_normalize.build_key", return_value="test-key"):
                    with patch("app.core.tags.tag_text", return_value=["Topic/Test"]):
                        with patch("app.core.commit_normalize.normalize_assignees", side_effect=lambda x, _: x):
                            
                            result = await parse_commit_text_async("Саша подготовит отчет", "Valentin")
    ...
    @pytest.mark.asyncio
    @patch("app.core.llm_commit_parse_async.get_async_openai_client")
    async def test_parse_commit_text_async_fallback(self, mock_get_client):
                ...
                with patch("app.core.commit_normalize.build_key", return_value="fallback-key"):
                    with patch("app.core.tags.tag_text", return_value=[]):
                        with patch("app.core.commit_normalize.normalize_assignees", side_effect=lambda x, _: x):
                            ...
                            result = await parse_commit_text_async("Тестовая задача", "Valentin")
...
class TestAsyncLLMExtractCommits:
    ...
    async def test_extract_commits_async_success(self, mock_get_client):
        ...
        result = await extract_commits_async(
            "Текст встречи", 
            ["Valentin", "Sasha"], 
            "2025-09-26"
        )
